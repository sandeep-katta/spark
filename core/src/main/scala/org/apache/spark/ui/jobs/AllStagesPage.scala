...
private[ui] class AllStagesPage(parent: StagesTab) extends WebUIPage("") {
  ...
  def render(request: HttpServletRequest): Seq[Node] = {
    ...
    val pools = if (sc.isDefined) {
      ...
      constructHistoryPools(parent.store.environmentInfo().poolInformation)
    }
  ...
  private def constructHistoryPools (
      poolInformation: Map[String, Schedulable]): Map[Schedulable, PoolData] = {
